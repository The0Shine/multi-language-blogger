...
import { AdminLanguageService } from '../../../language.service'; // đảm bảo đường dẫn đúng
...
export class AdminLanguageListComponent implements OnInit {
  ...
  loadLanguages(showSuccess = false) {
    this.languageService.getLanguages().subscribe((res) => {
      ...
      this.languages = langs.sort(
        (a: any, b: any) => a.languageid - b.languageid
      );
  ...
  confirmDeleteLanguage() {
    if (this.selectedLanguage) {
      this.languageService
        .permanentDeleteLanguage(this.selectedLanguage.languageid)
        .subscribe({
          next: () => {
            ...
            const currentPageData = this.filteredLanguages().slice(
              startIndex,
              endIndex
            );
