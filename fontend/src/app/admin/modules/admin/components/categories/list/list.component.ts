...
import { AdminCategoryService } from '../../../category.service'; // chỉnh lại nếu path khác
...
export class AdminCategorieListComponent implements OnInit {
  ...
  validateField(field: string) {
    if (field === 'category_name') {
      this.validationErrors.category_name =
        this.modalCategory.category_name.trim()
          ? ''
  ...
  confirmEditCategory() {
    ...
    this.categoryService
      .updateCategory(this.modalCategory.categoryid, payload)
      .subscribe({
        next: (res) => {
          if (res?.success && res.data) {
            reloadList();
